window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const zh = {
			...
			needConfirm: "需要确认",
			trustConnect: "信任并连接",
			grantsHint: "勾选后，当前项目下的对话可通过 SSHManager 与隧道使用这些主机。",
			saveGrants: "保存项目授权",
			saving: "保存中…",
		...
		const en = {
			...
			grantsHint: "Checked hosts can be used by SSHManager and the tunnel for this project.",
		...
		function ensureStyles() {
			...
			const STYLE_ID = "dsh-ssh-tunnel-style-v3";
			const prev = document.getElementById("dsh-ssh-tunnel-style") || document.getElementById("dsh-ssh-tunnel-style-v2") || document.getElementById("dsh-ssh-tunnel-style-v3");
			if (prev && prev.id === STYLE_ID && prev.getAttribute("data-rev") === "3") return;
			if (prev) prev.remove();
			const el = document.createElement("style");
			el.id = STYLE_ID;
			el.setAttribute("data-rev", "3");
			el.textContent = [
				...
				".ssh-t-row{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:12px 0;border-bottom:1px solid var(--dsw-alias-border-l1);color:var(--dsw-alias-label-primary);}",
				...
				React.createElement("div", { className: "ssh-t-root" },
					...
					view === "grants" ? React.createElement("div", { className: "ssh-t-card" },
						...
						hosts.map(function (h) {
							return React.createElement("label", { key: h.id, className: "ssh-t-check" },
								...
								React.createElement("span", null, (h.name || h.id) + " · " + h.username + "@" + h.host + " · " + h.credentialStatus),
							);
						}),
						React.createElement("div", { className: "ssh-t-actions" },
							React.createElement(Btn, { primary: true, disabled: !!busy, onClick: function () {
					...
					view === "hosts" ? React.createElement("div", { className: "ssh-t-card" },
						...
						hosts.map(function (h) {
							return React.createElement("div", { key: h.id, className: "ssh-t-row" },
								React.createElement("div", null,
									React.createElement("div", { style: { fontWeight: 600 } }, h.name || h.id),
									React.createElement("div", { className: "ssh-t-muted" }, h.username + "@" + h.host + ":" + h.port),
								),
								React.createElement("div", { className: "ssh-t-actions", style: { marginTop: 0 } },
						...
						scan ? React.createElement("div", { style: { marginTop: 12 } },
							(scan.configs || []).map(function (c, i) {
								return React.createElement("div", { key: i, className: "ssh-t-row" },
									React.createElement("div", null, c.name + " → " + c.host),
									React.createElement(Btn, { onClick: function () {
					...
					view === "tunnel" ? React.createElement("div", { className: "ssh-t-card" },
						...
						hosts.filter(function (h) { return granted.includes(h.id); }).map(function (h) {
							return React.createElement("div", { key: h.id, className: "ssh-t-row" },
								React.createElement("div", null, h.name || h.id),
								React.createElement(Btn, { primary: true, disabled: !!busy, onClick: function () {
						...
						sessions.length === 0
							...
							: sessions.map(function (s) {
								return React.createElement("div", { key: s.session_id, className: "ssh-t-row" },
									React.createElement("div", null,
										React.createElement("div", null, s.title || s.session_id),
										React.createElement("div", { className: "ssh-t-muted" }, s.status + " · sftp=" + String(s.sftpEnabled)),
									),
									React.createElement("div", { className: "ssh-t-actions", style: { marginTop: 0 } },
